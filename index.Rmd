...
The above chart, does not show enough information which makes sense because of the small percentage of the total variation. We could not see a good pattern or grouping in this chart.     
...
top10 <- top10 %>% select(country, mpg, am_abbr, ranking) 
```
...
find_model %>% filter(rank(desc(adjr), ties.method = "first") <= 10)
```

The above table, shows us the best model. Let's go ahead and see how it looks. 

```{r}
bestfit <- lm(data = mtcars, mpg ~ factor(am) + hp + wt + qsec)
